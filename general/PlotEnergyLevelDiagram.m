function PlotEnergyLevelDiagram(EnergyLevels,SystemDescription)
    ...
    figure;
    plot(EnergyLevels{1,1}*ones(1,length(EnergyLevels{1,2})),EnergyLevels{1,2},'*','LineWidth',1,'MarkerSize',10); hold on;
    for i=2:height(EnergyLevels)
        plot(EnergyLevels{i,1}*ones(1,length(EnergyLevels{i,2})),EnergyLevels{i,2},'*','LineWidth',1,'MarkerSize',10);
    end
    hold off;
