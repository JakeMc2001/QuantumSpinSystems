...
function Hterms=AnihoneycombHam(Jz,Jxy,Kz,Kxy,Gz,Gxy)
    tic
    Hterms = interactions;
    ...
    for i=1:length(xBonds)
        bond=xBonds(:,i);
        Hterms=Hterms.addHterm("SzSz",2,bond,Jxy);
        Hterms=Hterms.addHterm("SpSm",2,bond,(Jxy+Kxy)/2);
        Hterms=Hterms.addHterm("SmSp",2,bond,(Jxy+Kxy)/2);
    ...
    for i=1:length(zBonds)
        bond=zBonds(:,i);
        Hterms=Hterms.addHterm("SzSz",2,bond,Jz*Kz);
